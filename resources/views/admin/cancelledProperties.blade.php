    ...
    <div class="toolbar" id="kt_toolbar">
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Cancelled Properties</h1>
            </div>
        </div>
    </div>
    <!-- End::Toolbar -->
                        ...
                        <div class="card-body py-3">
                            <div class="table-responsive">
                                <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                                    ...
                                    <tbody id="cancel_property_table">
                                        @forelse($cancelled_properties as $property)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $property->property_number }}</td>
                                            <td>{{ $property->property_name }}</td>
                                            <td>{{ $property->landlord->user->name }}</td>
                                            <td>{{ $property->updated_at ? \Carbon\Carbon::parse($property->updated_at)->format('d-m-Y') : '-' }}</td> <!-- Updated column -->
                                            <td>
...
<script>
    function fetch_data(query = '', page = 1) {
        $.ajax({
            url: "{{ route('admin.searchCancelledProperties') }}", // The search route for cancelled properties
            method: 'GET',
            data: {
            ...
            success: function(data) {
                $('#cancel_property_table').html(data.table_data); // Update table rows
