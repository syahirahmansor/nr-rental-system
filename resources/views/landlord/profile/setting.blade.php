            ...
            <div id="kt_content_container" class="container-xxl">
                        ...
                        <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bolder">
                            ...
                            <li class="nav-item mt-2">
                                <a class="nav-link text-active-primary ms-0 me-10 py-5"
                                    href="{{ route('landlordprofile') }}">Overview</a>
                            </li>
                            <!--end::Nav item-->
                            <!--begin::Nav item-->
                            <li class="nav-item mt-2">
                ...
                <div class="card mb-5 mb-xl-10">
                    ...
                    <div id="kt_account_profile_details" class="collapse show">
                        <!--begin::Form-->
                        <form method="post" name="submit" enctype="multipart/form-data" id="kt_account_profile_details_form" class="form"
                            action="{{ route('landlordprofile.update', Auth::user()->id) }}">
                            @csrf
                            @method('put')
                            <!--begin::Card body-->
                            <div class="card-body border-top p-9">
                                ...
                                <div class="row mb-6">
                                    ...
                                    <div class="col-lg-8 fv-row">
                                        <input type="tel" name="phoneno"
                                            ...
                                            placeholder="Phone number" value="{{ Auth::user()->landlord->phoneno }}" />
                                        @error('phoneno')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-6">
                                    ...
                                    <div class="col-lg-8 fv-row">
                                        <textarea type="text" name="address" class="form-control form-control-lg form-control-solid"
                                            placeholder="Address">{{ Auth::user()->landlord->address }}</textarea>
                                        @error('address')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row mb-6">
                                    ...
                                    <div class="col-lg-8 fv-row">
                                        <select name="gender" aria-label="Select a Gender" data-control="select2"
                                            ...
                                            <option value="m"
                                                {{ Auth::user()->landlord->gender == 'm' ? 'selected' : '' }}>
                                                Male
                                            </option>
                                            <option value="f"
                                                {{ Auth::user()->landlord->gender == 'f' ? 'selected' : '' }}>
                                                Female</option>
                                        </select>
                                    </div>
                                    <!--end::Col-->
                                ...
                                <div class="row mb-6">
                                    ...
                                    <div class="col-lg-8 fv-row">
                                        <input type="date" name="dob"
                                            ...
                                            value="{{ \Carbon\Carbon::parse(Auth::user()->landlord->dob)->format('Y-m-d') }}" />
                                        @error('dob')
                                            <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <!--end::Input group-->
                                <!--begin::Input group for Document Upload--> 
                                <div class="row mb-6">
                                    <label class="col-lg-4 col-form-label fw-bold fs-6">Deed Documents</label>
                                    <div class="col-lg-8">
                                        <input type="file" name="documents[]" class="form-control form-control-lg form-control-solid mb-3 mb-lg-0" multiple />
                                        @error('documents.*')
                                        <div class="text-danger">{{ $message }}</div>
                                        @enderror
                                        <!-- Show the existing documents if available -->
                                         @if (Auth::user()->landlord->documents)
                                         <div class="mt-2">
                                            @foreach (Auth::user()->landlord->documents as $document)
                                            Current Documents: <a href="{{ asset('storage/' . $document->file_path) }}" target="_blank" class="fw-bolder fs-6 text-primary">{{ $document->original_name }}</a><br>
                                            @endforeach
                                        </div>
                                        @endif
                                    </div>
